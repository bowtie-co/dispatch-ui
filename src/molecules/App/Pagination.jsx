...
export const AppPagination = ({items, page = 1, perPage = 10, maxPageLinks, disableLast = false, setPage}) => {
  ...
  useEffect(() => {
    ...
    if (maxPageLinks) {
      let i;
      const half = Math.ceil(maxPageLinks / 2);
      if (page <= half) {
        i = 0;
      } else if (page >= pageCount - 2) {
        i = pageCount - maxPageLinks;
      } else {
        i = page - half;
      }
      setPageNumbers(getPages(pageCount).slice(i, i + maxPageLinks));
    } else {
      setPageNumbers(getPages(pageCount));
    }
  }, [ items, page, setPage, pageCount, perPage, maxPageLinks ]);
